...
First, download Form Block from WordPress.org:

<a class="epiphyt-button is-primary" href="https://wordpress.org/plugins/form-block/">Download Form Block</a>

After buying Form Block Pro you will receive a download link to the plugin file and a license key. Download the file as well. For the free Form Block plugin, such credentials are neither available, nor required.

## Installation through the backend

You can uploaded the downloaded `.zip` files directly in your WordPress backend. You will need a user account with administrator permissions (or super administrator permissions in multisite installations). As soon as you’re logged in, you can find the entry **Plugins > Install** in the menu. You can upload the `.zip` file on this page by clicking on the button “Upload plugin” alongside the headline.

## Installation through FTP

To install the downloaded `.zip` files through (S)FTP, you first need to extract it and upload the files and folders with an FTP program of your choice onto your website.  
After the upload, they should be found in `wp-content/plugins/form-block-pro` and `wp-content/plugins/form-block`.

## Installation via Composer <Badge type="info" text="Since 1.4.0" />
